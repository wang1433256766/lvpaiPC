...
use think\Log;
use think\Request;
use com\alipay\Pay\MD5;
use com\alipay\Pay\Core;
/* *
 * 类名：AlipayNotify
 * 功能：支付宝通知处理类
 * 详细：处理支付宝各接口通知返回
...
class Notify {
	...
	function verifyNotify(){
		...
		if(!$info) {//判断POST来的数组是否为空
			return false;
		}
		else {
			//生成签名结果
			$isSign = $this->getSignVeryfy($info, $info["sign"]);
			//获取支付宝远程服务器ATN结果（验证是否是支付宝发来的消息）
			$responseTxt = 'false';
			if (! empty($info["notify_id"])) {
				$responseTxt = $this->getResponse($info["notify_id"]);
			}
